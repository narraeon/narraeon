...
describe("世界提示框架可视化编辑器", () => {
  ...
  test("新增最近叙事插槽并调整条数，YAML 里落成 history slot", () => {
    ...
    // The count is editable, and history does not accept a required field.
  ...
  test("编辑当前情境绑定、指令顺序与确定性材料插槽", () => {
    ...
    // This fixture stores an id in the frame. The editor must resolve it to a
    // document so saving does not lose the binding, then write a visible @reference.
    ...
    fireEvent.click(screen.getByRole("button", { name: "下移 World Style" }));
    expect(
  ...
  test("目录与显式引用来源只从内容包中的实际关联目标选择", () => {
    ...
    expect(sourceOptions).toContain(
      "world/characters/alex.yaml — Alex · @character-alex",
    );
    ...
    fireEvent.change(source, { target: { value: "@character-alex" } });
    expect(latestFrame(changed).context[2]?.slot.from).toEqual({
      document: "@character-alex",
      locator: { yaml: [] },
    });
    expect(
      screen.getByText(
        "实际关联 world/characters/alex.yaml；frame.yaml 保存 @短引用：@character-alex",
      ),
    ).toBeTruthy();
  });

  // Every edit rewrites all of frame.yaml. Retaining ids would expose doc.<uuid>
  // values that later model edits cannot associate with any setting tool result.
...
context:
  ...
  - slot: { kind: document, document: character.alex }
...
`,
    ...
    // Only instruction order changes; document fields remain untouched.
    fireEvent.click(screen.getByRole("button", { name: "下移 World Style" }));

    const saved = latestFrame(changed);
    expect(saved.bindings.currentSituation).toBe("@current-situation");
    expect(saved.context[1]?.slot.document).toBe("@character-alex");
...
function latestFrame(
  ...
  if (typeof yaml !== "string")
    throw new Error("The test did not receive frame YAML");
...
function files(): ContentTreeFile[] {
  return [
    ...
    },
    {
      ...
      contents: document("character.alex", "Alex"),
    },
    {
    ...
    },
    {
      ...
      contents: "# World Style\n\nRestrained and specific.\n",
    },
    {
      path: "control/blocks/state.md",
      contents:
        "# State Maintenance\n\nWrite durable facts back to their natural owner.\n",
    },
  ];
}
