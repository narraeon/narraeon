...
3. 每个精确草稿版本的真实 Prompt Preview、机械诊断和完整差异；
4. 完整模型响应、Provider 原生续传、失败与应用结果。

流程：

1. 用户像普通对话一样直接说明当前想做什么；内容包正文不预先塞入消息，AI 通过固定工具从隔离草稿按需读取。
2. AI 从第一条消息开始就可以 list／search／read／write／patch／move 隔离草稿，并根据用户消息决定讨论、提问或修改；“先讨论计划”只是普通用户要求，不是 Runtime 阶段。
3. 修改前先完整读取既有文件；由 AI 创建或已经成功修改过的完整文件视为已读。
...
7. 每次草稿修订后，Runtime 自动检查开场白、文件安全、身份、引用、控制格式、必需当前情境、持久 selector 和真实 Prompt Preview；上下文是否可容纳请求由 Provider 判断。
