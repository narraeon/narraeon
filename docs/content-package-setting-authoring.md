...
开场与世界文档应停在同一时点。除持续事实外，当前情境还应保留仍待回应的具体请求、邀请或动作及说话者；原话决定含义时保存必要原话，使首次“好”“我摇头”有明确对象，不必复制整篇开场。
...
启铭对秦龙的看法写在启铭自己的文档中。两边可以不同；Runtime 不镜像、不创建关系 record，也不猜 `关系` 这个字段的特殊含义。创建后，关系可从“刚见面”逐渐表达具体共同经历、当前相处方式及仍有效的影响；不必改成固定 schema 或只留下好感等级。已兑现承诺退出待办，不抹去由此形成的信任，不将不同人物的理解作为重复事实删除。
...
AI 设定完善只编辑内容包，不修改已经创建世界的当前状态、已提交叙事或人物认知。运行中世界使用独立的世界修订入口，复用作者编排但维护当前时点的 state/control，不读取 opening.md、不恢复初始关系；应用后才提交世界。每段持久对话维护：

1. 用户与 AI 的 append-only 创作对话；
2. 每次主动发送冻结的当前玩法预设 revision、作者编排及已启用游玩语义只读参考，同次工具循环不换提示；模型续传兼容绑定仍属于整段会话；
...
`world/` 只保存已经成立的世界事实、不变量、认知、限制和持续倾向；人物档写刺激、选择倾向和约束，不写每次出场都必须表演的固定动作或镜头。怎样选择材料、裁决、揭露、更新或描写属于 `control/blocks/` 中本世界特有的作者控制；玩家可见小说语体只属于 `opening.md` 和未来游玩叙事。想要渐进揭露时，应保存当下谁知道、怀疑或误解什么、已有证据与现实阻力，不保存预定剧情阶段、必经场景或强制揭露顺序；人物实际意图、承诺、日程和疾病、仪式等真实机制可以作为当前事实存在，其未来目标不视为已经实现。
