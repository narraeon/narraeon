...
创建世界时，Runtime 会逐字把它写成第一条已提交 narrator 历史消息。它不会伪造一条玩家“开始游戏”，也不会让模型临场改写；生产游玩的全新上下文也不会把玩家已经看到的 genesis 再发给模型。之后修改源内容包的开场白不会改变既有世界；从旧端点派生时继承现有历史，不再增加第二份开场白。
...
如果 AI 写出 `好感度: 150`，Runtime 原样保存和显示。AI 可以在调用候选终态工具前认为它不符合剧情并继续修改，但不能把这种语义意见伪装成格式错误。
...
2. 可选的、面向用户的体验与文件计划；
3. 隔离的完整 YAML／Markdown 候选树；
4. 真实 Prompt Preview 和完整差异。

流程：

1. 用户可以选择这次完善必须先看到的文本文件；Runtime 从固定当前树读取完整原文并直接注入，浏览器不提交文件副本。
2. 采用计划优先路径时，AI 只能 list／search／read 现有文件，理解主要／次要体验、反复循环和边界后提出自然文档、规则文档、世界提示块和玩家视图计划，不设计 schema，也不能修改候选。
3. 用户可以确认计划后进入候选，也可以明确跳过可见计划直接生成候选；两条路径使用同一份固定当前树和隔离候选。
4. 候选阶段先检查 `opening.md`，再完整读取要修改的既有文件；已经直接注入的完整文件视为已读。
5. 开场白缺失时用专用工具创建；开局地点、人物、局面、语气或行动钩子变化时，完整读取后更新；不受影响时保留。
6. 小修改用 YAML 路径或 Markdown 标题区块 patch；新增自然对象才创建新文档。
7. Runtime 检查开场白、文件安全、身份、引用、控制格式、必需当前情境、持久 selector 和真实 Prompt Preview；上下文是否可容纳请求由 Provider 判断。
8. 真实编译器展示生产 bootstrap、开场白排除边界与首条玩家追加，并扫描内部 DTO 泄漏；无论是否跳过计划，用户仍需审阅并整批应用候选。
9. 用户整批确认后才原子替换内容包当前树；否则继续修订或整批放弃。
...
候选确认是整批替换，不做静默 merge、rebase 或部分勾选。创作期间若另有编辑，应用前界面必须让用户看到完整候选差异；当前版本不承诺自动合并。
