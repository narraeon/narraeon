...
运行中世界的手动编辑和 AI 创作统一进入一份 `state/*`／`control/*` 修订工作树。开启修订会从固定 Authority 端点和控制指纹创建或恢复一个持久 epoch，并以非过期独占锁阻止游玩及其他世界语义写入；本地名称不属于世界语义，因此仍可修改。`opening.md` 已经提交为 genesis 叙事，没有可供世界修订覆盖的当前文件。
